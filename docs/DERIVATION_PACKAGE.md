...
工程状态需与理论状态分开理解：`v6_lite_6` 已完成 P0——直接以 MuJoCo 精确碰撞几何加入目标 pair、移动目标外生漂移 CBF 和原生 500 Hz 离散重放门禁；V6.1-A 已实现解析 PCC、URDF 一致离散 FK、有限体积包络和三方影子距离审计，但尚未把新代理接入在线 QP，也尚无全状态域误差证书。因此本文后半的 PCC 推导现在已有影子实现依据，但仍不是当前在线控制的安全几何后端。
